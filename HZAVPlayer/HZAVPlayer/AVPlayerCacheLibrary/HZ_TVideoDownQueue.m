...
#import "HZ_TVideoDownQueue.h"
#import "HZ_TVideoDownOperation.h"
#import <libkern/OSAtomic.h>
@interface HZ_TVideoDownQueue()
@property (nonatomic,weak) HZ_TVideoDownOperation* currentDownLoadOperation;
@property (nonatomic,copy) NSDictionary* httpHeader;
@property (nonatomic,strong) HZ_TVideoFileManager* fileManager;
@end
@implementation HZ_TVideoDownQueue
{
...
//     __weak HZ_TVideoFileManager* wFileManager = _fileManager;
    ...
    for (NSArray* element  in segmentArr) {
        ...
        {
            HZ_TVideoDownOperation* requestOperation = [[HZ_TVideoDownOperation alloc]initWithUrl:_requestUrl withRange:NSMakeRange(start.unsignedIntegerValue, end.unsignedIntegerValue - start.unsignedIntegerValue + 1)];
            __weak HZ_TVideoDownOperation* wRequestOperation = requestOperation;
            ...
            [requestOperation setDownCompleteBk:^(NSError *error, NSUInteger offset, NSUInteger length) {
                ...
                if (error == nil  && self.assetResource.isFinished != true && self.assetResource.isCancelled != true && [self getCurrentOperaton] == 1) {
                ...
                if (error == nil) {
                  ...
                  self.isNetworkError = YES;
                    if ([self.delegate respondsToSelector:@selector(loadNetError:)]) {
                        [self cancelDownLoad];
                        [self.delegate loadNetError:wself];
                    }
                }
            }];
